...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto createItem(ItemDto itemDto, Long ownerId) {
        ...
        log.debug("Данные вещи: name='{}', description='{}', available={}, requestId={}",
        ...
        if (itemDto.getRequestId() != null) {
            log.debug("Ищем запрос с ID {} для связи", itemDto.getRequestId());
            ...
            log.info("Найден запрос: ID={}, description='{}', requestorId={}",
                    request.getId(), request.getDescription(), request.getRequestor().getId());
        }

        Item item = ItemMapper.toItem(itemDto, owner, request);
        log.debug("Создан объект Item (без ID)");

        Item savedItem = itemRepository.save(item);
        log.debug("Вещь сохранена в БД с ID: {}", savedItem.getId());

        if (request != null) {
            log.debug("Проверяем связь: request_id вещи = {}", savedItem.getRequest() != null ? savedItem.getRequest().getId() : "null");

            // вещь по request_id
            List<Item> itemsByRequest = itemRepository.findAllByRequestId(request.getId());
            log.debug("После сохранения найдено {} вещей с request_id = {}",
                    itemsByRequest.size(), request.getId());
            if (!itemsByRequest.isEmpty()) {
                log.debug("Найденные вещи: {}",
                        itemsByRequest.stream().map(Item::getId).collect(Collectors.toList()));
            }
        }
    ...
    public ItemDto getItemById(Long itemId, Long userId) {
        ...
        log.info("*** не владелец: {}", item.getOwner());
        // Для не-владельцев возвращаем только вещь с комментариями
    ...
    public CommentDto addComment(Long itemId, CreateCommentDto commentDto, Long authorId) {
        ...
        log.info("*** создание комента для item: {}", item);
...
//        }
        ...
        if (!hasBookerEver) {
            throw new ValidationException("Комментировать можно только когда-либо бронированные вещи");
        }
